...
/**
 * The Class OddLastYieldFunction.
 */
public class OddLastYieldFunction implements FreeRefFunction {
	...
	public ValueEval evaluate(ValueEval[] args, OperationEvaluationContext ec) {
		...
		try {
			...
			ValueEval valueEval8 = null;
			
			if (args.length == 8)
				valueEval8 = OperandResolver.getSingleValue(args[7], ec.getRowIndex(), ec.getColumnIndex());
			...
			Date lastInterest = DateUtil.getJavaDate(OperandResolver.coerceValueToDouble(valueEval3));

			double rate = OperandResolver.coerceValueToDouble(valueEval4);

			double price = OperandResolver.coerceValueToDouble(valueEval5);

			double redemption = OperandResolver.coerceValueToDouble(valueEval6);

			int frequency = OperandResolver.coerceValueToInt(valueEval7);

			int basis = 0;

			if (valueEval8 != null)
				basis = OperandResolver.coerceValueToInt(valueEval8);
			
			return new NumberEval(AnalysisToolPak.getOddLYield(settlement, maturity, lastInterest, rate, price, redemption, frequency, basis));
