...
/**
 * The Class AccruedInterestFunction.
 */
public class AccruedInterestFunction implements FreeRefFunction {
	...
	public ValueEval evaluate(ValueEval[] args, OperationEvaluationContext ec) {
		...
		try {
			...
			if (args.length > 5)
				valueEval6 = OperandResolver.getSingleValue(args[5], ec.getRowIndex(), ec.getColumnIndex());

			ValueEval valueEval7 = null;
			if (args.length > 6)
				valueEval7 = OperandResolver.getSingleValue(args[6], ec.getRowIndex(), ec.getColumnIndex());

			Date settlement = DateUtil.getJavaDate(OperandResolver.coerceValueToDouble(valueEval3));

			Date issue = DateUtil.getJavaDate(OperandResolver.coerceValueToDouble(valueEval1));

			Date firstInterestDate = DateUtil.getJavaDate(OperandResolver.coerceValueToDouble(valueEval2));

			double rate = OperandResolver.coerceValueToDouble(valueEval4);

			double par = 1000;

			double valueEval5Double = OperandResolver.coerceValueToDouble(valueEval5);

			double frequency = 0;

			int basis = 0;

			if (valueEval5Double != 1d && valueEval5Double != 2d && valueEval5Double != 4d) {
				par = valueEval5Double;
				if (valueEval6 != null) {
					frequency = OperandResolver.coerceValueToDouble(valueEval6);
					if (valueEval7 != null)
				...
				frequency = valueEval5Double;
				if (valueEval6 != null)
					basis = OperandResolver.coerceValueToInt(valueEval6);
			}

			return new NumberEval(AnalysisToolPak.getAccrInt(issue,firstInterestDate, settlement, rate, par,(int)frequency, basis));
