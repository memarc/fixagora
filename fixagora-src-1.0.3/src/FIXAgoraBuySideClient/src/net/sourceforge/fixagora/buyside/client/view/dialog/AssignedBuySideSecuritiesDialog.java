...
/**
 * The Class AssignedBuySideSecuritiesDialog.
 */
public class AssignedBuySideSecuritiesDialog extends JDialog {
	 ...
	 *
	 * @param assignedBuySideBookSecurities the assigned buy side book securities
	 * @param bankCalendars the bank calendars
	 * @param edit the edit
	 */
	public AssignedBuySideSecuritiesDialog(List<AssignedBuySideBookSecurity> assignedBuySideBookSecurities, List<BankCalendar> bankCalendars, boolean edit) {

		this.assignedBuySideBookSecurities = assignedBuySideBookSecurities;

		setBounds(100, 100, 487, 277);
		setBackground(new Color(204, 216, 255));
		setIconImage(new ImageIcon(AssignedBuySideSecuritiesDialog.class.getResource("/net/sourceforge/fixagora/basis/client/view/images/16x16/a-logo.png"))
				.getImage());
		setModal(true);
		...
		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getValuta() != null)
			valutaTextField.setText(integerFormat.format(assignedBuySideBookSecurities.get(0).getValuta()));
		else
			valutaTextField.setText(integerFormat.format(assignedBuySideBookSecurities.get(0).getBuySideBook().getValuta()));
		...
		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getBankCalendar() != null)
			bankCalendarComboBox.setSelectedItem(assignedBuySideBookSecurities.get(0).getBankCalendar());
		else
			bankCalendarComboBox.setSelectedItem(assignedBuySideBookSecurities.get(0).getBuySideBook().getBankCalendar());
		...
		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getDaycountConvention() != null)
			daycountComboBox.setSelectedItem(new ComboBoxEntry(assignedBuySideBookSecurities.get(0).getDaycountConvention(), null));
		else
			daycountComboBox.setSelectedItem(new ComboBoxEntry(assignedBuySideBookSecurities.get(0).getBuySideBook().getDaycountConvention(), null));
		...
		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getCalcMethod() != null)
			calcMethod = assignedBuySideBookSecurities.get(0).getCalcMethod();
		else if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getBuySideBook().getCalcMethod() != null)
			calcMethod = assignedBuySideBookSecurities.get(0).getBuySideBook().getCalcMethod();
		...
		if (assignedBuySideBookSecurities.get(0).getBuySideBook().getFractionalDisplay() != null
				&& assignedBuySideBookSecurities.get(0).getBuySideBook().getFractionalDisplay() == true)
			displayPriceComboBox.setSelectedIndex(1);

		if (assignedBuySideBookSecurities.get(0).getFractionalDisplay() != null) 
		{
			if(assignedBuySideBookSecurities.get(0).getFractionalDisplay() == true)
				displayPriceComboBox.setSelectedIndex(1);
			else
				displayPriceComboBox.setSelectedIndex(0);
		}
		...
		okButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (AssignedBuySideBookSecurity assignedBuySideBookSecurity : AssignedBuySideSecuritiesDialog.this.assignedBuySideBookSecurities) {
					if (!assignedBuySideBookSecurity.getBuySideBook().getDaycountConvention()
							.equals(((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry()))
						assignedBuySideBookSecurity.setDaycountConvention((Integer) ((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry());
					else
						assignedBuySideBookSecurity.setDaycountConvention(null);
					
					if(!assignedBuySideBookSecurity.getBuySideBook().getFractionalDisplay().equals(((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry()))
						assignedBuySideBookSecurity.setFractionalDisplay((Boolean) ((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry());
					else
						assignedBuySideBookSecurity.setFractionalDisplay(null);
					
					if (assignedBuySideBookSecurity.getBuySideBook().getCalcMethod().ordinal() != ((Integer) ((ComboBoxEntry) calculationComboBox
							.getSelectedItem()).getEntry()))
						switch (calculationComboBox.getSelectedIndex()) {
							case 1:
								assignedBuySideBookSecurity.setCalcMethod(CalcMethod.DEFAULT);
								break;
							case 2:
								assignedBuySideBookSecurity.setCalcMethod(CalcMethod.DISC);
								break;
							case 3:
								assignedBuySideBookSecurity.setCalcMethod(CalcMethod.MAT);
								break;
							case 4:
								assignedBuySideBookSecurity.setCalcMethod(CalcMethod.ODDF);
								break;
							case 5:
								assignedBuySideBookSecurity.setCalcMethod(CalcMethod.ODDL);
								break;
							case 6:
								assignedBuySideBookSecurity.setCalcMethod(CalcMethod.TBILL);
								break;
							default:
								assignedBuySideBookSecurity.setCalcMethod(CalcMethod.NONE);
								break;
						}
					else
						assignedBuySideBookSecurity.setCalcMethod(null);


					if (!assignedBuySideBookSecurity.getBuySideBook().getBankCalendar().equals(bankCalendarComboBox.getSelectedItem()))
						assignedBuySideBookSecurity.setBankCalendar((BankCalendar) bankCalendarComboBox.getSelectedItem());
					else
						assignedBuySideBookSecurity.setBankCalendar(null);
					...
					if (!assignedBuySideBookSecurity.getBuySideBook().getValuta().equals(value))
						assignedBuySideBookSecurity.setValuta(value);
					else
						assignedBuySideBookSecurity.setValuta(null);
				}
	...
	private void checkConsistency() {
		...
		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getDaycountConvention() != null
				&& !assignedBuySideBookSecurities.get(0).getDaycountConvention().equals(((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry()))
			dirty = true;

		if (assignedBuySideBookSecurities.size() > 0
				&& assignedBuySideBookSecurities.get(0).getDaycountConvention() == null
				&& !assignedBuySideBookSecurities.get(0).getBuySideBook().getDaycountConvention()
						.equals(((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry()))
			dirty = true;
		
		if (assignedBuySideBookSecurities.size() > 0 &&assignedBuySideBookSecurities.get(0).getFractionalDisplay() != null && !assignedBuySideBookSecurities.get(0).getFractionalDisplay().equals(((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry()))
			dirty = true;
				
		if (assignedBuySideBookSecurities.size() > 0 &&assignedBuySideBookSecurities.get(0).getFractionalDisplay() == null && !assignedBuySideBookSecurities.get(0).getBuySideBook().getFractionalDisplay().equals(((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry()))
			dirty = true;

		if (assignedBuySideBookSecurities.size() > 0
				&& assignedBuySideBookSecurities.get(0).getCalcMethod() != null
				&& assignedBuySideBookSecurities.get(0).getCalcMethod().ordinal() != ((Integer) ((ComboBoxEntry) calculationComboBox.getSelectedItem())
						.getEntry()))
			dirty = true;

		if (assignedBuySideBookSecurities.size() > 0
				&& assignedBuySideBookSecurities.get(0).getCalcMethod() == null
				&& assignedBuySideBookSecurities.get(0).getBuySideBook().getCalcMethod().ordinal() != ((Integer) ((ComboBoxEntry) calculationComboBox
						.getSelectedItem()).getEntry()))
			dirty = true;

		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getBankCalendar() != null
				&& !assignedBuySideBookSecurities.get(0).getBankCalendar().equals(bankCalendarComboBox.getSelectedItem()))
			;
		dirty = true;

		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getBankCalendar() == null
				&& !assignedBuySideBookSecurities.get(0).getBuySideBook().getBankCalendar().equals(bankCalendarComboBox.getSelectedItem()))
			dirty = true;
		...
		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getValuta() != null
				&& !assignedBuySideBookSecurities.get(0).getValuta().equals(value))
			;
		dirty = true;

		if (assignedBuySideBookSecurities.size() > 0 && assignedBuySideBookSecurities.get(0).getValuta() == null
				&& !assignedBuySideBookSecurities.get(0).getBuySideBook().getValuta().equals(value))
			dirty = true;
