...
/**
 * The Class BuySideBookTableRenderer.
 */
public class BuySideBookTableRenderer implements TableCellRenderer {
	...
	/**
	 * Instantiates a new buy side book table renderer.
	 *
	 * @param buySideBook the buy side book
	 */
	public BuySideBookTableRenderer(BuySideBook buySideBook) {

		super();
		
		this.buySideBook  = buySideBook;

		switch (buySideBook.getDisplayStyle()) {
			case 0:
			...
			case 1:
				...
				foreGroundDefault = new Color(250, 196, 0);	 //new Color(255, 153, 0);
				green = new Color(0, 224, 64); //new Color(0, 204, 0);
				red = new Color(250, 117, 47);	 //new Color(255, 51, 51);
	...
	public Component getTableCellRendererComponent(final JTable table, final Object value, final boolean isSelected, final boolean hasFocus, final int row,
		...
		BuySideBookEditorMonitorTableModel buySideBookEditorMonitorTableModel = (BuySideBookEditorMonitorTableModel) table.getModel();
		
		if(column>5&&!buySideBookEditorMonitorTableModel.isShowYield())
			column++;

		if (buySideBookEditorMonitorTableModel.getMouseOverRow() == table.convertRowIndexToModel(row)&&column!=0)
		...
		if (value != null)
		{
			...
			if(column==4)
			{
				component.setBorder(new CompoundBorder(new MatteBorder(border, 0, border, border2, borderColor), new EmptyBorder(4, 5, 4, 5+buySideBookEditorMonitorTableModel.getLimitOffset(value.toString()))));
			}
			if(column==5)
			{
				component.setBorder(new CompoundBorder(new MatteBorder(border, 0, border, border2, borderColor), new EmptyBorder(4, 5, 4, 5+buySideBookEditorMonitorTableModel.getLastOffset(value.toString()))));
			}
		}
		...
		if (column == 0 && value instanceof String) {
			...
			if(buySideBook.getDisplayStyle()==1)
				backGround = new Color(250, 196, 0);
			
			component.setForeground(Color.WHITE);
			
			if(buySideBookEditorMonitorTableModel.isCompleteFilled(table.convertRowIndexToModel(row)))
			{
			...
			else if(buySideBookEditorMonitorTableModel.isCanceled(table.convertRowIndexToModel(row)))
			{
			...
			if(buySideBookEditorMonitorTableModel.isUpdated(table.convertRowIndexToModel(row)))
