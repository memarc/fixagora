...
/**
 * The Class BuySideBookEditorMasterData.
 */
public class BuySideBookEditorMasterData extends JScrollPane {
	 ...
	 */
	public BuySideBookEditorMasterData(BuySideBookEditor buySideBookEditor) {
		...
		nameWarningLabel = buySideBookEditor.getNameWarningLabel();
		...
		bankCalendarComboBox = new JComboBox(buySideBookEditor.getMainPanel().getBankCalendars().toArray());
		...
		gbc_styleComboBox.fill = GridBagConstraints.HORIZONTAL;
		gbc_styleComboBox.insets = new Insets(0, 0, 5, 5);
	...
	private void checkConsistency() {
		...
		if (nameTextField.getText().trim().length() == 0) {
			...
			nameWarningLabel.setIcon(buySideBookEditor.getBugIcon());
			...
			if (buySideBookEditor.getAbstractBusinessObject().getName() == null
					|| !buySideBookEditor.getAbstractBusinessObject().getName().equals(nameTextField.getText())) {

				buySideBookEditor.checkName(nameTextField.getText());
				dirty = true;
			}
		}

		if (dirtyFieldCheck(buySideBookEditor.getBuySideBook().getValuta(), valutaTextField))
			dirty = true;

		if (buySideBookEditor.getBuySideBook().getBankCalendar() != null
				&& !buySideBookEditor.getBuySideBook().getBankCalendar().equals(bankCalendarComboBox.getSelectedItem()))
			dirty = true;

		if (buySideBookEditor.getBuySideBook().getBankCalendar() == null)
			dirty = true;

		if (buySideBookEditor.getBuySideBook().getDaycountConvention() != null
				&& !buySideBookEditor.getBuySideBook().getDaycountConvention().equals(((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry()))
			dirty = true;

		if (buySideBookEditor.getBuySideBook().getDaycountConvention() == null)
			dirty = true;
		
		if (buySideBookEditor.getBuySideBook().getCalcMethod()!=null&&buySideBookEditor.getBuySideBook().getCalcMethod().ordinal()!=((Integer)((ComboBoxEntry) calculationComboBox.getSelectedItem()).getEntry()))
			dirty = true;
		
		if (buySideBookEditor.getBuySideBook().getCalcMethod() == null)
			dirty = true;
		
		if (buySideBookEditor.getBuySideBook().getAbsolutChange()!=null&&!buySideBookEditor.getBuySideBook().getAbsolutChange().equals(((ComboBoxEntry) displayChangeComboBox.getSelectedItem()).getEntry()))
			dirty = true;
		
		if (buySideBookEditor.getBuySideBook().getAbsolutChange() == null)
			dirty = true;
		
		if(buySideBookEditor.getBuySideBook().getDisplayStyle()!=styleComboBox.getSelectedIndex())
			dirty=true;
		
		if (buySideBookEditor.getBuySideBook().getFractionalDisplay()!=null&&!buySideBookEditor.getBuySideBook().getFractionalDisplay().equals(((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry()))
			dirty = true;
		...
		if (textArea.getText().trim().length() == 0) {
			...
			descriptionWarningLabel.setIcon(buySideBookEditor.getWarningIcon());
		}
		else {
		...
		if (buySideBookEditor.getBuySideBook().getDescription() != null && !buySideBookEditor.getBuySideBook().getDescription().equals(textArea.getText())) {
			
			dirty = true;
		}
		
		if (buySideBookEditor.getBuySideBook().getDescription() == null && textArea.getText().trim().length()>0) {
	...
	public void save() {
		...
		buySideBookEditor.getBuySideBook().setName(nameTextField.getText().trim());
		buySideBookEditor.getBuySideBook().setDaycountConvention((Integer) ((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry());
		buySideBookEditor.getBuySideBook().setBankCalendar((BankCalendar) bankCalendarComboBox.getSelectedItem());
		buySideBookEditor.getBuySideBook().setDisplayStyle(styleComboBox.getSelectedIndex());
		
		if(displayPriceComboBox.getSelectedIndex()==0)
			buySideBookEditor.getBuySideBook().setFractionalDisplay(false);
		else
			buySideBookEditor.getBuySideBook().setFractionalDisplay(true);
		
		switch(calculationComboBox.getSelectedIndex())
		{
			case 1:
				buySideBookEditor.getBuySideBook().setCalcMethod(CalcMethod.DEFAULT);
				break;
			case 2:
				buySideBookEditor.getBuySideBook().setCalcMethod(CalcMethod.DISC);
				break;
			case 3:
				buySideBookEditor.getBuySideBook().setCalcMethod(CalcMethod.MAT);
				break;
			case 4:
				buySideBookEditor.getBuySideBook().setCalcMethod(CalcMethod.ODDF);
				break;
			case 5:
				buySideBookEditor.getBuySideBook().setCalcMethod(CalcMethod.ODDL);
				break;
			case 6:
				buySideBookEditor.getBuySideBook().setCalcMethod(CalcMethod.TBILL);
				break;
			default:
				buySideBookEditor.getBuySideBook().setCalcMethod(CalcMethod.NONE);
				break;
		}
		
		if(displayChangeComboBox.getSelectedIndex()==0)
			buySideBookEditor.getBuySideBook().setAbsolutChange(true);
		else
			buySideBookEditor.getBuySideBook().setAbsolutChange(false);
		
		if (valutaTextField.getText().trim().length() > 0)
			buySideBookEditor.getBuySideBook().setValuta(Integer.parseInt(valutaTextField.getText()));
		else
			buySideBookEditor.getBuySideBook().setValuta(0);
		buySideBookEditor.getBuySideBook().setDescription(textArea.getText());
	}
	...
	public void updateContent() {
		...
		nameTextField.setText(buySideBookEditor.getBuySideBook().getName());
		
		textArea.setText(buySideBookEditor.getBuySideBook().getDescription());
		
		styleComboBox.setSelectedIndex(buySideBookEditor.getBuySideBook().getDisplayStyle());

		daycountComboBox.setSelectedItem(new ComboBoxEntry(buySideBookEditor.getBuySideBook().getDaycountConvention(), null));
		
		if(buySideBookEditor.getBuySideBook().getCalcMethod()!=null)
			switch(buySideBookEditor.getBuySideBook().getCalcMethod())
			{
				case DEFAULT:
					calculationComboBox.setSelectedIndex(1);
					break;
		...
		if(buySideBookEditor.getBuySideBook().getAbsolutChange()!=null)
		{
			if(buySideBookEditor.getBuySideBook().getAbsolutChange()==true)
				displayChangeComboBox.setSelectedIndex(0);
			else
		...
		if(buySideBookEditor.getBuySideBook().getFractionalDisplay()!=null)
		{
			if(buySideBookEditor.getBuySideBook().getFractionalDisplay()==true)
		...
		if (buySideBookEditor.getBuySideBook().getBankCalendar() != null)
			bankCalendarComboBox.setSelectedItem(buySideBookEditor.getBuySideBook().getBankCalendar());
		else
			bankCalendarComboBox.setSelectedIndex(0);

		if (buySideBookEditor.getBuySideBook().getValuta() != null)
			valutaTextField.setText(integerFormat.format(buySideBookEditor.getBuySideBook().getValuta()));
		else
			valutaTextField.setText("0");

		if (buySideBookEditor.getAbstractBusinessObject().getId() != 0) {

			StringBuffer textBuffer = new StringBuffer();
			textBuffer.append(DateFormat.getTimeInstance().format(buySideBookEditor.getAbstractBusinessObject().getModificationDate()));
			textBuffer.append(" ");
			textBuffer.append(DateFormat.getDateInstance(DateFormat.SHORT).format(buySideBookEditor.getAbstractBusinessObject().getModificationDate()));

			if (buySideBookEditor.getAbstractBusinessObject().getModificationUser() != null) {

				textBuffer.append(" by ");
				textBuffer.append(buySideBookEditor.getAbstractBusinessObject().getModificationUser());
			}
		...
		if (buySideBookEditor.getBasisClientConnector().getFUser().canWrite(buySideBookEditor.getAbstractBusinessObject())
				&& !(buySideBookEditor.getAbstractBusinessObject().getName() != null && buySideBookEditor.getAbstractBusinessObject().getName()
						.equals("Admin Role"))) {
