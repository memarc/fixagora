...
import net.sourceforge.fixagora.basis.client.model.editor.TraderPartyIDTableModel;
import net.sourceforge.fixagora.basis.client.view.GradientTableHeaderRenderer;
import net.sourceforge.fixagora.basis.client.view.dialog.TraderPartyIDDialog;
import net.sourceforge.fixagora.basis.shared.model.persistence.TraderPartyID;

/**
 * The Class TraderEditorPartyID.
 */
public class TraderEditorPartyID extends JPanel {

	private JButton addButton;
	private JTable table;
	private TraderEditor traderEditor;
	private TraderPartyIDTableModel traderPartyIDTableModel;
	private JButton removeButton;
	private JScrollPane scrollPane;
	private JButton editButton;

	/**
	 * Instantiates a new trader editor party id.
	 *
	 * @param traderEditor the trader editor
	 */
	public TraderEditorPartyID(final TraderEditor traderEditor) {
		...
		addButton.setIcon(new ImageIcon(TraderEditorPartyID.class.getResource("/net/sourceforge/fixagora/basis/client/view/images/16x16/edit-add.png")));
		...
		traderPartyIDTableModel = new TraderPartyIDTableModel(traderEditor);
		table.setModel(traderPartyIDTableModel);
		...
		table.setDefaultRenderer(Object.class, new DefaultEditorTableRenderer(traderEditor));
		...
		traderPartyIDTableModel.setTable(table);
		...
		removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectionModel().getMinSelectionIndex() != -1
						&& traderEditor.getBasisClientConnector().getFUser().canWrite(traderEditor.getAbstractBusinessObject())) {

					List<TraderPartyID> traderPartyIDs = new ArrayList<TraderPartyID>();
					for (int index : table.getSelectedRows())
						traderPartyIDs.add(traderPartyIDTableModel.get(index));
					for (TraderPartyID traderPartyID : traderPartyIDs)
						traderPartyIDTableModel.remove(traderPartyID);
					checkConsistency();
				}
			}
		});
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (traderEditor.getBasisClientConnector().getFUser().canWrite(traderEditor.getAbstractBusinessObject())) {
					...
					TraderPartyIDDialog traderPartyIDDialog = new TraderPartyIDDialog(null, traderEditor.getMainPanel().getBusinessComponents());
					traderPartyIDDialog.setVisible(true);
					if (!traderPartyIDDialog.isCancelled()) {
						TraderPartyID traderPartyID = traderPartyIDDialog.getTraderPartyID();
						traderPartyID.setTrader(traderEditor.getTrader());
						traderPartyIDTableModel.addTraderPartyID(traderPartyID);
						checkConsistency();
					}
				}
			}
		...
		editButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectionModel().getMinSelectionIndex() != -1
					...
					TraderPartyIDDialog traderPartyIDDialog = new TraderPartyIDDialog(traderPartyIDTableModel.get(table.getSelectionModel()
							.getMinSelectionIndex()), traderEditor.getMainPanel().getBusinessComponents());
					traderPartyIDDialog.setVisible(true);
					if (!traderPartyIDDialog.isCancelled()) {
						traderPartyIDTableModel.modified();
						traderPartyIDTableModel.fireTableDataChanged();
					}
				}
			}
		});
		...
		scrollPane.addComponentListener(new ComponentListener() {
			...
			public void componentResized(final ComponentEvent e) {
				...
				traderPartyIDTableModel.setMinWidth((int) scrollPane.getSize().getWidth() - 3);
			}
	...
	public boolean isDirty() {
		...
		return traderPartyIDTableModel.isDirty();
	}
	...
	public void updateContent() {
		...
		traderPartyIDTableModel.updateTraderPartyIDs(traderEditor.getTrader().getTraderPartyIDs());
