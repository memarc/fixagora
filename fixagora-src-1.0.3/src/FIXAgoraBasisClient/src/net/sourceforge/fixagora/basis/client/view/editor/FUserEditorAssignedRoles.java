...
/**
 * The Class FUserEditorAssignedRoles.
 */
public class FUserEditorAssignedRoles extends JScrollPane {
	...
	/**
	 * Instantiates a new f user editor assigned roles.
	 *
	 * @param fUserEditor the f user editor
	 * @param roles the roles
	 */
	public FUserEditorAssignedRoles(final FUserEditor fUserEditor, List<FRole> roles) {

		super();

		this.userEditor = fUserEditor;
		...
		for (FRole fRole : fUserEditor.getUser().getAssignedRoles())
			defaultListModel.addElement(fRole);
	...
	private void checkDirty() {
		...
		if (userEditor.getUser().getAssignedRoles().containsAll(objects) && objects.containsAll(userEditor.getUser().getAssignedRoles()))
			dirty = false;
	...
	public void save() {
		...
		Set<FRole> objects = new HashSet<FRole>();

		for (int i = 0; i < assignedList.getModel().getSize(); i++)
			objects.add((FRole) assignedList.getModel().getElementAt(i));

		userEditor.getUser().setAssignedRoles(objects);
	}
	...
	public void updateContent() {
		...
		List<FRole> roles = userEditor.getRoles();
		roles.removeAll(userEditor.getUser().getAssignedRoles());
		
		for(FRole role: roles)
		{
			if(!defaultListModel.contains(role)&&userEditor.getBasisClientConnector().getFUser().canRead(role))
				defaultListModel2.addElement(role);
		}
		
		defaultListModel.removeAllElements();
		for(FRole role: userEditor.getUser().getAssignedRoles())
			defaultListModel.addElement(role);
		...
		if (assignedList.getSelectedValues().length > 0 && userEditor.getBasisClientConnector().getFUser().canWrite(userEditor.getAbstractBusinessObject()))
			removeButton.setEnabled(true);
		
		else
			removeButton.setEnabled(false);
		
		if (notAssignedList.getSelectedValues().length > 0 && userEditor.getBasisClientConnector().getFUser().canWrite(userEditor.getAbstractBusinessObject()))
			addButton.setEnabled(true);
		...
		removeAllButton.setEnabled(assignedList.getModel().getSize() > 0
				&& userEditor.getBasisClientConnector().getFUser().canWrite(userEditor.getAbstractBusinessObject()));
