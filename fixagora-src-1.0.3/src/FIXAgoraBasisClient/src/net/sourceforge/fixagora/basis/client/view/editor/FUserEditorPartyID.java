...
import net.sourceforge.fixagora.basis.client.model.editor.FUserPartyIDTableModel;
import net.sourceforge.fixagora.basis.client.view.GradientTableHeaderRenderer;
import net.sourceforge.fixagora.basis.client.view.dialog.FUserPartyIDDialog;
import net.sourceforge.fixagora.basis.shared.model.persistence.FUserPartyID;

/**
 * The Class FUserEditorPartyID.
 */
public class FUserEditorPartyID extends JPanel {

	private JButton addButton;
	private JTable table;
	private FUserEditor fUserEditor;
	private FUserPartyIDTableModel fUserPartyIDTableModel;
	private JButton removeButton;
	private JScrollPane scrollPane;
	private JButton editButton;

	/**
	 * Instantiates a new f user editor party id.
	 *
	 * @param fUserEditor the f user editor
	 */
	public FUserEditorPartyID(final FUserEditor fUserEditor) {
		...
		addButton.setIcon(new ImageIcon(FUserEditorPartyID.class.getResource("/net/sourceforge/fixagora/basis/client/view/images/16x16/edit-add.png")));
		...
		fUserPartyIDTableModel = new FUserPartyIDTableModel(fUserEditor);
		table.setModel(fUserPartyIDTableModel);
		...
		table.setDefaultRenderer(Object.class, new DefaultEditorTableRenderer(fUserEditor));
		...
		fUserPartyIDTableModel.setTable(table);
		...
		removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectionModel().getMinSelectionIndex() != -1
						&& fUserEditor.getBasisClientConnector().getFUser().canWrite(fUserEditor.getAbstractBusinessObject())) {
					
					List<FUserPartyID> fUserPartyIDs = new ArrayList<FUserPartyID>();
					for(int index: table.getSelectedRows())
						fUserPartyIDs.add(fUserPartyIDTableModel.get(index));
					for(FUserPartyID fUserPartyID: fUserPartyIDs)
						fUserPartyIDTableModel.remove(fUserPartyID);
					checkConsistency();
				}
			}
		});
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fUserEditor.getBasisClientConnector().getFUser().canWrite(fUserEditor.getAbstractBusinessObject())) {
					...
					FUserPartyIDDialog fUserPartyIDDialog = new FUserPartyIDDialog(null, fUserEditor.getMainPanel()
							.getBusinessComponents());
					fUserPartyIDDialog.setVisible(true);
					if (!fUserPartyIDDialog.isCancelled()) {
						FUserPartyID fuserPartyID = fUserPartyIDDialog.getFUserPartyID();
						fuserPartyID.setFuser(fUserEditor.getUser());
						fUserPartyIDTableModel.addFUserPartyID(fuserPartyID);
						checkConsistency();
					}
				}
			}
		...
		editButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectionModel().getMinSelectionIndex() != -1
					...
					FUserPartyIDDialog fUserPartyIDDialog = new FUserPartyIDDialog(fUserPartyIDTableModel.get(table.getSelectionModel().getMinSelectionIndex()), fUserEditor.getMainPanel()
							.getBusinessComponents());
					fUserPartyIDDialog.setVisible(true);
					if (!fUserPartyIDDialog.isCancelled()) {
						fUserPartyIDTableModel.modified();
						fUserPartyIDTableModel.fireTableDataChanged();
					}
				}
			}
		});
		...
		scrollPane.addComponentListener(new ComponentListener() {
			...
			public void componentResized(final ComponentEvent e) {
				...
				fUserPartyIDTableModel.setMinWidth((int) scrollPane.getSize().getWidth() - 3);
			}
	...
	public boolean isDirty() {
		...
		return fUserPartyIDTableModel.isDirty();
	}
	...
	public void updateContent() {
		...
		fUserPartyIDTableModel.updateFuserPartyIDs(fUserEditor.getUser().getfUserPartyIDs());
