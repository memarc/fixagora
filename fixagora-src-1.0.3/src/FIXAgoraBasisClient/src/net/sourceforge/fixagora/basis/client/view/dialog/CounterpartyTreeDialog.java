...
/**
 * The Class CounterpartyTreeDialog.
 */
public class CounterpartyTreeDialog extends JDialog {
	 ...
	 */
	public CounterpartyTreeDialog(BusinessObjectTreeNode topNode, BasisClientConnector basisClientConnector, JDialog jDialog) {
		super(jDialog);
	 ...
	 * @param basisClientConnector the basis client connector
	 */
	public CounterpartyTreeDialog(BusinessObjectTreeNode topNode, BasisClientConnector basisClientConnector) {
		super();
		init(topNode, basisClientConnector);
	}
	...
	private void init(BusinessObjectTreeNode topNode, BasisClientConnector basisClientConnector)
	{
		...
		setIconImage(new ImageIcon(CounterpartyTreeDialog.class.getResource("/net/sourceforge/fixagora/basis/client/view/images/16x16/a-logo.png")).getImage());
		setModal(true);
			...
			public void mouseClicked(final MouseEvent e) {
				...
				if (e.getClickCount() == 2) {
					...
					if (path != null) {
						...
						if (node.getUserObject() instanceof Counterparty) {

							counterparty = (Counterparty) node.getUserObject();
							setVisible(false);
						}
					}
				}
	...
	private void checkConsistency() {
		...
		okButton.setEnabled(counterparty != null);
	}
	...
	/**
	 * Gets the counterparty list.
	 *
	 * @return the counterparty list
	 */
	public List<String> getCounterpartyList() {
		...
		for (BusinessObjectTreeNode defaultMutableTreeNode : defaultMutableTreeNodeMap.values())
			...
			if (defaultMutableTreeNode.getUserObject() instanceof Counterparty)
				counterparties.add(defaultMutableTreeNode.getUserObject().toString());
	...
	/**
	 * Gets the counterparty for name.
	 *
	 * @param name the name
	 * @return the counterparty for name
	 */
	public Counterparty getCounterpartyForName(String name) {
		...
		for (BusinessObjectTreeNode defaultMutableTreeNode : defaultMutableTreeNodeMap.values())
			...
			if (defaultMutableTreeNode.getUserObject() instanceof Counterparty) {
				...
				Counterparty counterparty = (Counterparty) defaultMutableTreeNode.getUserObject();

				if (counterparty.getName().trim().equals(name.trim()))
					return counterparty;
			}
	...
	/**
	 * Gets the counterparty for id.
	 *
	 * @param id the id
	 * @return the counterparty for id
	 */
	public Counterparty getCounterpartyForId(long id) {
	...
	/**
	 * Gets the filtered counterparty tree tree dialog.
	 *
	 * @param counterparties the counterparties
	 * @param newOrderSingleDialog the new order single dialog
	 * @return the filtered counterparty tree tree dialog
	 */
	public CounterpartyTreeDialog getFilteredCounterpartyTreeTreeDialog(Set<Counterparty> counterparties, JDialog newOrderSingleDialog) {
		BusinessObjectTreeNode dialogNode = new BusinessObjectTreeNode(topNode.getUserObject());
		copySubTree(dialogNode, topNode, counterparties);
		while(cleanSubTree(dialogNode));
		return new CounterpartyTreeDialog(dialogNode, basisClientConnector,newOrderSingleDialog);
	}
	 ...
	 * @param counterparties the counterparties
	 */
	public void copySubTree(BusinessObjectTreeNode subRoot, BusinessObjectTreeNode sourceTree, Set<Counterparty> counterparties) {
		...
		for (int i = 0; i < sourceTree.getChildCount(); i++) {
			...
			if(counterparties.contains(child.getUserObject())||!(child.getUserObject() instanceof Counterparty))
			{
				BusinessObjectTreeNode clone = new BusinessObjectTreeNode(child.getUserObject());
				subRoot.add(clone);
				copySubTree(clone, child, counterparties);
			}
		}
		return;
	}
	...
	public boolean cleanSubTree(BusinessObjectTreeNode subRoot) {
		...
		if(subRoot.getParent()!=null&&subRoot.getChildCount()==0&&!(subRoot.getUserObject() instanceof Counterparty))
		{
