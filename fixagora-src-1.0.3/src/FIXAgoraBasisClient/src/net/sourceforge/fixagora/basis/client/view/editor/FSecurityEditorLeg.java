...
import net.sourceforge.fixagora.basis.client.model.dnd.SecurityLegDropTarget;
...
import net.sourceforge.fixagora.basis.client.view.dialog.SecurityLegDialog;
import net.sourceforge.fixagora.basis.shared.model.persistence.SecurityLeg;

/**
 * The Class FSecurityEditorLeg.
 */
public class FSecurityEditorLeg extends JPanel {
	 ...
	 */
	public FSecurityEditorLeg(final FSecurityEditor fSecurityEditor) {
		...
		securityLegTableModel = new SecurityLegTableModel(securityEditor);
		table.setModel(securityLegTableModel);
		...
		new SecurityLegDropTarget(table, securityEditor, this, securityLegTableModel);
		
		securityLegTableModel.setTable(table);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectionModel().getMinSelectionIndex() != -1
					...
					for(int index: table.getSelectedRows())
						securityLegs.add(securityLegTableModel.get(index));
					for(SecurityLeg securityLeg: securityLegs)
						securityLegTableModel.remove(securityLeg);
					checkConsistency();
				}
			}
		});
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fSecurityEditor.getMainPanel().getSecurityTreeDialog().getSecurityList().size() > 0) {
					SecurityLegDialog securityLegDialog = new SecurityLegDialog(null, fSecurityEditor);
					securityLegDialog.setVisible(true);
					if (!securityLegDialog.isCancelled()) {
						SecurityLeg securityLeg = securityLegDialog.getSecurityLeg();
		...
		editButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectionModel().getMinSelectionIndex() != -1
					...
					SecurityLegDialog securityLegDialog = new SecurityLegDialog(securityLegTableModel.get(table.getSelectionModel()
							.getMinSelectionIndex()), fSecurityEditor);
					securityLegDialog.setVisible(true);
					if (!securityLegDialog.isCancelled()) {
						securityLegTableModel.modified();
						securityLegTableModel.fireTableDataChanged();
						checkConsistency();
					}
				}
			}
		...
		scrollPane.addComponentListener(new ComponentListener() {
			...
			public void componentResized(final ComponentEvent e) {
				...
				securityLegTableModel.setMinWidth((int) scrollPane.getSize().getWidth() - width - 3);
			}
	...
	public boolean isDirty() {
		...
		return securityLegTableModel.isDirty();
	}
	...
	public void updateContent() {
		...
		securityLegTableModel.updateSecurityLegs(securityEditor.getSecurity().getSecurityDetails().getSecurityLegs());
