...
/**
 * The Class FUserPartyIDTableModel.
 */
public class FUserPartyIDTableModel extends AbstractTableModel {
	...
	private List<FUserPartyID> fUserPartyIDs = null;

	private boolean modified = false;

	/**
	 * Instantiates a new f user party id table model.
	 *
	 * @param fUserEditor the f user editor
	 */
	public FUserPartyIDTableModel(final FUserEditor fUserEditor) {

		super();

		this.fUserEditor = fUserEditor;

		updateFuserPartyIDs(this.fUserEditor.getUser().getfUserPartyIDs());
	...
	public Object getValueAt(final int rowIndex, final int columnIndex) {
		...
		FUserPartyID fUserPartyID = fUserPartyIDs.get(rowIndex);
		switch (columnIndex) {
			case 1:
				return fUserPartyID.getPartyID();

			case 2:
				if (fUserPartyID.getPartyIDSource() == null)
					return "not used";
				else if (fUserPartyID.getPartyIDSource().equals("B"))
					return "BIC (Bank Identification Code)";
				else if (fUserPartyID.getPartyIDSource().equals("C"))
					return "Generally accepted market participant identifier";
				else if (fUserPartyID.getPartyIDSource().equals("D"))
					return "Proprietary / Custom code";
				else if (fUserPartyID.getPartyIDSource().equals("E"))
					return "ISO Country Code";
				else if (fUserPartyID.getPartyIDSource().equals("F"))
					return "Settlement Entity Location";
				else if (fUserPartyID.getPartyIDSource().equals("G"))
					return "MIC (ISO 10383 - Market Identificer Code)";
				else if (fUserPartyID.getPartyIDSource().equals("H"))
					return "CSD participant/member code";
				else if (fUserPartyID.getPartyIDSource().equals("6"))
					return "UK National Insurance or Pension Number";
				else if (fUserPartyID.getPartyIDSource().equals("7"))
					return "US Social Security Number";
				else if (fUserPartyID.getPartyIDSource().equals("8"))
					return "US Employer or Tax ID Number";
				else if (fUserPartyID.getPartyIDSource().equals("9"))
					return "Australian Business Number";
				else if (fUserPartyID.getPartyIDSource().equals("A"))
					return "Australian Tax File Number";
				else if (fUserPartyID.getPartyIDSource().equals("I"))
					return "Directed broker three character acronym as defined in ISITC";
				else if (fUserPartyID.getPartyIDSource().equals("1"))
					return "Korean Investor ID";
				else if (fUserPartyID.getPartyIDSource().equals("2"))
					return "Taiwanese Qualified Foreign Investor ID QFII/FID";
				else if (fUserPartyID.getPartyIDSource().equals("3"))
					return "Taiwanese Trading Acct";
				else if (fUserPartyID.getPartyIDSource().equals("4"))
					return "Malaysian Central Depository (MCD) number";
				else if (fUserPartyID.getPartyIDSource().equals("5"))
					return "Chinese Investor ID";
				else
					return "unknown";

			case 3:
				if (fUserPartyID.getPartyRole() == null)
					return "all";
				else
					switch (fUserPartyID.getPartyRole()) {
						case 1:
			...
			default:
				if (fUserPartyID.getAbstractBusinessComponent() == null)
					return "Default";
				return fUserPartyID.getAbstractBusinessComponent().getName();
		}
	...
	public boolean isDirty() {
		...
		boolean clean = !modified && fUserPartyIDs.containsAll(fUserEditor.getUser().getfUserPartyIDs())
				&& fUserEditor.getUser().getfUserPartyIDs().containsAll(fUserPartyIDs);
	...
	public void save() {
		...
		fUserEditor.getUser().setfUserPartyIDs(fUserPartyIDs);
	}
	 ...
	 * @param list the list
	 */
	public void updateFuserPartyIDs(List<FUserPartyID> list) {

		this.fUserPartyIDs = new ArrayList<FUserPartyID>();

		if (list != null)
			this.fUserPartyIDs.addAll(list);
	 ...
	 *
	 * @param fUserPartyID the f user party id
	 */
	public void addFUserPartyID(FUserPartyID fUserPartyID) {

		fUserPartyIDs.add(fUserPartyID);
		checkDoubleEntries();
		fireTableDataChanged();
	}
	 ...
	 * @param fUserPartyID the f user party id
	 */
	public void remove(FUserPartyID fUserPartyID) {

		fUserPartyIDs.remove(fUserPartyID);
		fireTableDataChanged();
	}
	 ...
	 * @return the f user party id
	 */
	public FUserPartyID get(int index) {

		return fUserPartyIDs.get(index);
	}
	...
	private void checkDoubleEntries() {
		...
		List<FUserPartyID> fUserPartyIDs2 = new ArrayList<FUserPartyID>();
		fUserPartyIDs2.addAll(this.fUserPartyIDs);
		for (FUserPartyID fUserPartyID : fUserPartyIDs2) {
			List<FUserPartyID> fUserPartyIDs3 = new ArrayList<FUserPartyID>();
			fUserPartyIDs3.addAll(this.fUserPartyIDs);
			for (FUserPartyID fUserPartyID2 : fUserPartyIDs3)
				if (fUserPartyID != fUserPartyID2 && fUserPartyID.getAbstractBusinessComponent() == null
						&& fUserPartyID2.getAbstractBusinessComponent() == null)
					fUserPartyIDs.remove(fUserPartyID);
				else if (fUserPartyID != fUserPartyID2
						&& fUserPartyID.getAbstractBusinessComponent()!=null && fUserPartyID2.getAbstractBusinessComponent()!=null
						&& fUserPartyID.getAbstractBusinessComponent().getId() == fUserPartyID2.getAbstractBusinessComponent().getId()
						&& fUserPartyID.getFuser().getId() == fUserPartyID2.getFuser().getId()
						&& optionalValueCheck(fUserPartyID.getPartyRole(), fUserPartyID2.getPartyRole())
						&& optionalValueCheck(fUserPartyID.getPartyIDSource(), fUserPartyID2.getPartyIDSource()))
					fUserPartyIDs.remove(fUserPartyID);
		}

		Collections.sort(fUserPartyIDs);
