...
import net.sourceforge.agora.simulator.model.ComplexEventDate;
import net.sourceforge.agora.simulator.model.ComplexEventTime;
import net.sourceforge.agora.simulator.model.Security;
import net.sourceforge.agora.simulator.model.SecurityAltIDGroup;
import net.sourceforge.agora.simulator.model.SecurityAttribute;
import net.sourceforge.agora.simulator.model.SecurityComplexEvent;
import net.sourceforge.agora.simulator.model.SecurityDetails;
import net.sourceforge.agora.simulator.model.SecurityEvent;
import net.sourceforge.agora.simulator.model.SecurityLeg;
import net.sourceforge.agora.simulator.model.SecurityUnderlying;
...
public class SecurityDictionary {
	...
	Map<String, Security> originalSecurityMap = new HashMap<String, Security>();

	/** The id security map. */
	Map<String, Security> idSecurityMap = new HashMap<String, Security>();

	/** The alternative security map. */
	Map<String, Map<String, Security>> alternativeSecurityMap = new HashMap<String, Map<String, Security>>();
	...
	private static SecurityDictionary securityDictionary = null;

	private SecurityDictionary() {
	...
	public synchronized void removeSecurity(Security security) {
		...
		for (Entry<String, Security> entry : originalSecurityMap.entrySet())
			if (entry.getValue().getSecurityID().equals(security.getSecurityID()))
				toRemove.add(entry.getKey());
		for (String id : toRemove)
			originalSecurityMap.remove(id);

		idSecurityMap.remove(security.getSecurityID());

		List<String> sources = new ArrayList<String>();
		sources.addAll(alternativeSecurityMap.keySet());

		for (String idSource : sources) {
			Map<String, Security> securityMap = alternativeSecurityMap.get(idSource);
			List<String> toRemove2 = new ArrayList<String>();
			for (Entry<String, Security> entry : securityMap.entrySet())
				if (entry.getValue().getSecurityID().equals(security.getSecurityID()))
	 ...
	 * @param security the security
	 */
	public synchronized void addSecurity(Security security) {

		originalSecurityMap.put(security.getSecurityID(), security);
		idSecurityMap.put(security.getSecurityID(), security);
		for (SecurityAltIDGroup securityAltIDGroup : security.getSecurityDetails().getSecurityAltIDGroups()) {
			Map<String, Security> securityMap = alternativeSecurityMap.get(securityAltIDGroup.getSecurityAltIDSource());
			if (securityMap == null) {
				securityMap = new HashMap<String, Security>();
	 ...
	 * @param security the security
	 */
	public synchronized void updateSecurity(Security security) {
	 ...
	 */
	public synchronized Security getSecurityForDefaultSecurityID(String securityID, String idSource) {

		if (securityID == null || idSource == null)
			return null;

		Security defaultSecurity = originalSecurityMap.get(securityID);
	 ...
	 * @return the security for default security id
	 */
	public synchronized Security getSecurityForDefaultSecurityID(String securityID) {
	 ...
	 */
	public synchronized Security getSecurityForBusinessObjectID(long securityID) {
	 ...
	 */
	public synchronized Group getRelatedSym(Security fSecurity) {
		...
		final Group group = new Group(146, 55, new int[] { 55, 65, 48, 22, 454, 460, 461, 167, 762, 200, 541, 224, 225, 239, 226, 227, 228, 255, 543, 470, 471,
				...
				221, 222, 662, 663, 699, 761, 235, 236, 701, 696, 697, 698, 561, 562, 336, 625, 827, 58, 354, 355 });
		group.setString(55, fSecurity.getName());
		if (fSecurity.getSecurityDetails().getSymbolSfx() != null)
			group.setString(65, fSecurity.getSecurityDetails().getSymbolSfx());
		if (fSecurity.getSecurityID() != null)
			group.setString(48, fSecurity.getSecurityID());
		if (fSecurity.getSecurityDetails().getSecurityIDSource() != null)
		...
		for (SecurityAltIDGroup securityAltIDGroup : fSecurity.getSecurityDetails().getSecurityAltIDGroups()) {
			...
			if (securityAltIDGroup.getSecurityAltID() != null)
				securityAltID.setString(455, securityAltIDGroup.getSecurityAltID());
			if (securityAltIDGroup.getSecurityAltIDSource() != null)
		...
		if (fSecurity.getSecurityDetails().getcFICode() != null)
			group.setString(461, fSecurity.getSecurityDetails().getcFICode());
		if (fSecurity.getSecurityDetails().getSecurityType() != null)
			group.setString(167, fSecurity.getSecurityDetails().getSecurityType());
		if (fSecurity.getSecurityDetails().getSecuritySubType() != null)
		...
		if (fSecurity.getSecurityDetails().getCreditRating() != null)
			group.setString(255, fSecurity.getSecurityDetails().getCreditRating());
		if (fSecurity.getSecurityDetails().getInstrumentRegistry() != null)
			group.setString(543, fSecurity.getSecurityDetails().getInstrumentRegistry());
		if (fSecurity.getSecurityDetails().getCountryOfIssue() != null)
			group.setString(470, fSecurity.getSecurityDetails().getCountryOfIssue());
		if (fSecurity.getSecurityDetails().getStateOfIssue() != null)
			group.setString(471, fSecurity.getSecurityDetails().getStateOfIssue());
		if (fSecurity.getSecurityDetails().getLocaleOfIssue() != null)
		...
		if (fSecurity.getSecurityDetails().getStrikeCurrency() != null)
		...
		if (fSecurity.getSecurityDetails().getSecurityExchange() != null)
			group.setString(207, fSecurity.getSecurityDetails().getSecurityExchange());
		if (fSecurity.getSecurityDetails().getIssuer() != null)
			group.setString(106, fSecurity.getSecurityDetails().getIssuer());
		if (fSecurity.getSecurityDetails().getDescription() != null)
		...
		if (fSecurity.getSecurityDetails().getCpRegType() != null)
		...
		for (SecurityEvent securityEvent : fSecurity.getSecurityDetails().getSecurityEvents()) {
			final Group securityEventGroup = new Group(864, 865, new int[] { 865, 866, 867, 868 });
			...
			if (securityEvent.getEventText() != null)
		...
		if (fSecurity.getSecurityDetails().getSecurityStatus() != null)
			group.setString(965, fSecurity.getSecurityDetails().getSecurityStatus());

		if (fSecurity.getSecurityDetails().getSettlOnOpen() != null)
		...
		if (fSecurity.getSecurityDetails().getUnitOfMeasure() != null)
			group.setString(996, fSecurity.getSecurityDetails().getUnitOfMeasure());

		if (fSecurity.getSecurityDetails().getTimeUnit() != null)
		...
		if (fSecurity.getSecurityDetails().getSecurityGroup() != null)
		...
		if (fSecurity.getSecurityDetails().getProductComplex() != null)
		...
		if (fSecurity.getSecurityDetails().getSettleMethod() != null)
		...
		if (fSecurity.getSecurityDetails().getPriceQuoteMethod() != null)
			group.setString(1196, fSecurity.getSecurityDetails().getPriceQuoteMethod());

		if (fSecurity.getSecurityDetails().getListMethod() != null)
			group.setInt(1198, fSecurity.getSecurityDetails().getListMethod());
		...
		if (fSecurity.getSecurityDetails().getValuationMethod() != null)
		...
		if (fSecurity.getSecurityDetails().getRestructuringType() != null)
			group.setString(1449, fSecurity.getSecurityDetails().getRestructuringType());

		if (fSecurity.getSecurityDetails().getSeniority() != null)
		...
		for (SecurityAttribute securityAttribute : fSecurity.getSecurityDetails().getSecurityAttribute()) {
			...
			if (securityAttribute.getAttributeValue() != null)
		...
		for (SecurityUnderlying securityUnderlying : fSecurity.getSecurityDetails().getSecurityUnderlyings()) {
			...
			Security underlying = securityUnderlying.getUnderlyingSecurity();

			securityUnderlyingGroup.setString(311, underlying.getName());
			if (underlying.getSecurityID() != null)
				securityUnderlyingGroup.setString(309, underlying.getSecurityID());
			if (underlying.getSecurityDetails().getSecurityIDSource() != null)
		...
		if (fSecurity.getSecurityDetails().getCurrency() != null)
		...
		for (SecurityLeg securityLeg : fSecurity.getSecurityDetails().getSecurityLegs()) {
			...
			Security underlying = securityLeg.getLegSecurity();

			securityLegGroup.setString(600, underlying.getName());
			if (underlying.getSecurityID() != null)
	 ...
	 * @return the related sym2
	 */
	public synchronized Security getRelatedSym2(Group group) {

		try {
			Security fSecurity = new Security();
			...
			for (int i = 1; i <= group.getGroupCount(1483); i++) {
				...
				for (int j = 1; j <= securityComplexEventGroup.getGroupCount(1483); j++) {
			...
			for (int i = 1; i <= group.getGroupCount(711); i++) {
				...
				Security underlying = getSecurityForDefaultSecurityID(securityID, securityIDSource);
			...
			for (int i = 1; i <= group.getGroupCount(555); i++) {
				...
				if (securityLegGroup.isSetField(600))
					securityID = securityLegGroup.getString(600);
				if (securityLegGroup.isSetField(602))
					securityIDSource = securityLegGroup.getString(602);

				Security leg = getSecurityForDefaultSecurityID(securityID, securityIDSource);
	 ...
	 *
	 * @return single instance of SecurityDictionary
	 */
	public static SecurityDictionary getInstance() {

		if (securityDictionary == null)
			securityDictionary = new SecurityDictionary();
