...
/**
 * The Class SellSideQuotePageEditorAssignedSecurity.
 */
public class SellSideQuotePageEditorAssignedSecurity extends JScrollPane {
	...
	/**
	 * Instantiates a new sell side quote page editor assigned security.
	 *
	 * @param sellSideQuotePageEditor the sell side quote page editor
	 */
	public SellSideQuotePageEditorAssignedSecurity(final SellSideQuotePageEditor sellSideQuotePageEditor) {
		...
		DefaultListCellRenderer defaultListCellRenderer = new DefaultListCellRenderer()
		{
			...
			public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
				...
				if(value instanceof AssignedSellSideBookSecurity)
				{
					
					FSecurity security = (FSecurity)sellSideQuotePageEditor.getMainPanel().getAbstractBusinessObjectForId(((AssignedSellSideBookSecurity)value).getSecurity().getId());
					if(security!=null&&security.getParent()!=null)
					{
						AbstractBusinessObject abstractBusinessObject = sellSideQuotePageEditor.getMainPanel().getAbstractBusinessObjectForId(security.getParent().getId());
		...
		notAssignedList.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (notAssignedList.getSelectedValues().length > 0
						&& sellSideQuotePageEditor.getBasisClientConnector().getFUser().canWrite(sellSideQuotePageEditor.getAbstractBusinessObject()))
		...
		for (AssignedSellSideBookSecurity assignedSellSideBookSecurity : sellSideQuotePageEditor.getSellSideQuotePage().getAssignedSellSideBookSecurities())
			defaultListModel.addElement(assignedSellSideBookSecurity);
	...
	private void checkDirty() {
		...
		if (sellSideQuotePageEditor.getSellSideQuotePage().getAssignedSellSideBookSecurities().containsAll(objects) && objects.containsAll(sellSideQuotePageEditor.getSellSideQuotePage().getAssignedSellSideBookSecurities()))
			dirty = false;

		else
			dirty = true;

		sellSideQuotePageEditor.checkDirty();
	}
	...
	public void save() {
		...
		List<AssignedSellSideBookSecurity> objects = new ArrayList<AssignedSellSideBookSecurity>();

		for (int i = 0; i < assignedList.getModel().getSize(); i++)
			objects.add((AssignedSellSideBookSecurity) assignedList.getModel().getElementAt(i));

		sellSideQuotePageEditor.getSellSideQuotePage().setAssignedSellSideBookSecurities(objects);
		for(AssignedSellSideBookSecurity assignedSellSideBookSecurity: sellSideQuotePageEditor.getSellSideQuotePage().getAssignedSellSideBookSecurities())
			assignedSellSideBookSecurity.getSellSideQuotePages().add(sellSideQuotePageEditor.getSellSideQuotePage());
	...
	public void updateContent() {
		...
		SellSideBook sellSideBook = (SellSideBook) sellSideQuotePageEditor.getMainPanel().getAbstractBusinessObjectForId(sellSideQuotePageEditor.getSellSideQuotePage().getParent().getId());
		
		List<AssignedSellSideBookSecurity> assignedSellSideBookSecurities = new ArrayList<AssignedSellSideBookSecurity>(sellSideBook.getAssignedSellSideBookSecurities());
		assignedSellSideBookSecurities.removeAll(sellSideQuotePageEditor.getSellSideQuotePage().getAssignedSellSideBookSecurities());
		...
		{
			FSecurity fSecurity = (FSecurity)sellSideQuotePageEditor.getMainPanel().getAbstractBusinessObjectForId(assignedSellSideBookSecurity.getSecurity().getId());
			if(!defaultListModel.contains(assignedSellSideBookSecurity)&&sellSideQuotePageEditor.getBasisClientConnector().getFUser().canRead(fSecurity));
				defaultListModel2.addElement(assignedSellSideBookSecurity);
		}
		
		defaultListModel.removeAllElements();
		
		List<AssignedSellSideBookSecurity> assignedSellSideBookSecurities2 = new ArrayList<AssignedSellSideBookSecurity>(sellSideQuotePageEditor.getSellSideQuotePage().getAssignedSellSideBookSecurities());
		
		for(AssignedSellSideBookSecurity assignedSellSideBookSecurity: assignedSellSideBookSecurities2)
			defaultListModel.addElement(assignedSellSideBookSecurity);
		

		if (sellSideQuotePageEditor.getBasisClientConnector().getFUser().canWrite(sellSideQuotePageEditor.getAbstractBusinessObject())) {
		...
		if (assignedList.getSelectedValues().length > 0 && sellSideQuotePageEditor.getBasisClientConnector().getFUser().canWrite(sellSideQuotePageEditor.getAbstractBusinessObject()))
			removeButton.setEnabled(true);
		
		else
			removeButton.setEnabled(false);
		
		if (notAssignedList.getSelectedValues().length > 0 && sellSideQuotePageEditor.getBasisClientConnector().getFUser().canWrite(sellSideQuotePageEditor.getAbstractBusinessObject()))
			addButton.setEnabled(true);
