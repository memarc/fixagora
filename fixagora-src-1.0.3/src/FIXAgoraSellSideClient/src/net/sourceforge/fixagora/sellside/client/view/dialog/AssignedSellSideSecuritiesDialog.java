...
/**
 * The Class AssignedSellSideSecuritiesDialog.
 */
public class AssignedSellSideSecuritiesDialog extends JDialog {
	...
	/**
	 * Instantiates a new assigned sell side securities dialog.
	 *
	 * @param assignedSellSideBookSecurities the assigned sell side book securities
	 * @param bankCalendars the bank calendars
	 * @param edit the edit
	 */
	public AssignedSellSideSecuritiesDialog(List<AssignedSellSideBookSecurity> assignedSellSideBookSecurities, List<BankCalendar> bankCalendars, boolean edit) {

		this.assignedSellSideBookSecurities = assignedSellSideBookSecurities;

		setBounds(100, 100, 487, 276);
		setBackground(new Color(204, 216, 255));
		setIconImage(new ImageIcon(AssignedSellSideSecuritiesDialog.class.getResource("/net/sourceforge/fixagora/basis/client/view/images/16x16/a-logo.png"))
				.getImage());
		setModal(true);
		...
		gbl_contentPanel.columnWidths = new int[] { 0, 0, 0, 52 };
		...
		gbc_positionLimitLabel.insets = new Insets(25, 25, 5, 5);
		gbc_positionLimitLabel.gridx = 0;
		gbc_positionLimitLabel.gridy = 0;
		...
		gbc_positionLimitTextField.insets = new Insets(25, 0, 5, 25);
		...
		gbc_positionLimitTextField.gridy = 0;
		contentPanel.add(valutaTextField, gbc_positionLimitTextField);

		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getValuta() != null)
			valutaTextField.setText(integerFormat.format(assignedSellSideBookSecurities.get(0).getValuta()));
		else
			valutaTextField.setText(integerFormat.format(assignedSellSideBookSecurities.get(0).getSellSideBook().getValuta()));
		...
		gbc_bankCalendarComboBox.insets = new Insets(0, 0, 5, 25);
		...
		gbc_bankCalendarComboBox.gridy = 1;
		contentPanel.add(bankCalendarComboBox, gbc_bankCalendarComboBox);

		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getBankCalendar() != null)
			bankCalendarComboBox.setSelectedItem(assignedSellSideBookSecurities.get(0).getBankCalendar());
		else
			bankCalendarComboBox.setSelectedItem(assignedSellSideBookSecurities.get(0).getSellSideBook().getBankCalendar());
		...
		gbc_priceBoundaryMethodComboBox.gridy = 2;
		contentPanel.add(daycountComboBox, gbc_priceBoundaryMethodComboBox);

		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getDaycountConvention() != null)
			daycountComboBox.setSelectedItem(new ComboBoxEntry(assignedSellSideBookSecurities.get(0).getDaycountConvention(), null));
		else
			daycountComboBox.setSelectedItem(new ComboBoxEntry(assignedSellSideBookSecurities.get(0).getSellSideBook().getDaycountConvention(), null));
		...
		gbc_calculationComboBox.insets = new Insets(0, 0, 5, 25);
		gbc_calculationComboBox.fill = GridBagConstraints.HORIZONTAL;
		gbc_calculationComboBox.gridx = 2;
		gbc_calculationComboBox.gridy = 3;
		contentPanel.add(calculationComboBox, gbc_calculationComboBox);

		CalcMethod calcMethod = CalcMethod.DEFAULT;

		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getCalcMethod() != null)
			calcMethod = assignedSellSideBookSecurities.get(0).getCalcMethod();
		else if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getSellSideBook().getCalcMethod() != null)
			calcMethod = assignedSellSideBookSecurities.get(0).getSellSideBook().getCalcMethod();
		...
		gbc_displayPriceComboBox.insets = new Insets(0, 0, 5, 25);
		gbc_displayPriceComboBox.fill = GridBagConstraints.HORIZONTAL;
		gbc_displayPriceComboBox.gridx = 2;
		gbc_displayPriceComboBox.gridy = 4;
		contentPanel.add(displayPriceComboBox, gbc_displayPriceComboBox);

		displayPriceComboBox.setSelectedIndex(0);

		if (assignedSellSideBookSecurities.get(0).getSellSideBook().getFractionalDisplay() != null
				&& assignedSellSideBookSecurities.get(0).getSellSideBook().getFractionalDisplay() == true)
			displayPriceComboBox.setSelectedIndex(1);

		if (assignedSellSideBookSecurities.get(0).getFractionalDisplay() != null) 
		{
			if(assignedSellSideBookSecurities.get(0).getFractionalDisplay() == true)
				displayPriceComboBox.setSelectedIndex(1);
			else
				displayPriceComboBox.setSelectedIndex(0);
		}
		...
		okButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (AssignedSellSideBookSecurity assignedSellSideBookSecurity : AssignedSellSideSecuritiesDialog.this.assignedSellSideBookSecurities) {
					if (!assignedSellSideBookSecurity.getSellSideBook().getDaycountConvention()
							.equals(((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry()))
						assignedSellSideBookSecurity.setDaycountConvention((Integer) ((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry());
					else
						assignedSellSideBookSecurity.setDaycountConvention(null);
					
					if(!assignedSellSideBookSecurity.getSellSideBook().getFractionalDisplay().equals(((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry()))
						assignedSellSideBookSecurity.setFractionalDisplay((Boolean) ((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry());
					else
						assignedSellSideBookSecurity.setFractionalDisplay(null);


					if (assignedSellSideBookSecurity.getSellSideBook().getCalcMethod().ordinal() != ((Integer) ((ComboBoxEntry) calculationComboBox
							.getSelectedItem()).getEntry()))
						switch (calculationComboBox.getSelectedIndex()) {
							case 1:
								assignedSellSideBookSecurity.setCalcMethod(CalcMethod.DEFAULT);
								break;
							case 2:
								assignedSellSideBookSecurity.setCalcMethod(CalcMethod.DISC);
								break;
							case 3:
								assignedSellSideBookSecurity.setCalcMethod(CalcMethod.MAT);
								break;
							case 4:
								assignedSellSideBookSecurity.setCalcMethod(CalcMethod.ODDF);
								break;
							case 5:
								assignedSellSideBookSecurity.setCalcMethod(CalcMethod.ODDL);
								break;
							case 6:
								assignedSellSideBookSecurity.setCalcMethod(CalcMethod.TBILL);
								break;
							default:
								assignedSellSideBookSecurity.setCalcMethod(CalcMethod.NONE);
								break;
						}
					else
						assignedSellSideBookSecurity.setCalcMethod(null);

					if (!assignedSellSideBookSecurity.getSellSideBook().getBankCalendar().equals(bankCalendarComboBox.getSelectedItem()))
						assignedSellSideBookSecurity.setBankCalendar((BankCalendar) bankCalendarComboBox.getSelectedItem());
					else
						assignedSellSideBookSecurity.setBankCalendar(null);
					...
					if (!assignedSellSideBookSecurity.getSellSideBook().getValuta().equals(value))
						assignedSellSideBookSecurity.setValuta(value);
					else
						assignedSellSideBookSecurity.setValuta(null);
				}
	...
	private void checkConsistency() {
		...
		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getDaycountConvention() != null
				&& !assignedSellSideBookSecurities.get(0).getDaycountConvention().equals(((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry()))
			dirty = true;

		if (assignedSellSideBookSecurities.size() > 0
				&& assignedSellSideBookSecurities.get(0).getDaycountConvention() == null
				&& !assignedSellSideBookSecurities.get(0).getSellSideBook().getDaycountConvention()
						.equals(((ComboBoxEntry) daycountComboBox.getSelectedItem()).getEntry()))
			dirty = true;
		
		if (assignedSellSideBookSecurities.size() > 0 &&assignedSellSideBookSecurities.get(0).getFractionalDisplay() != null && !assignedSellSideBookSecurities.get(0).getFractionalDisplay().equals(((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry()))
			dirty = true;
				
		if (assignedSellSideBookSecurities.size() > 0 &&assignedSellSideBookSecurities.get(0).getFractionalDisplay() == null && !assignedSellSideBookSecurities.get(0).getSellSideBook().getFractionalDisplay().equals(((ComboBoxEntry) displayPriceComboBox.getSelectedItem()).getEntry()))
			dirty = true;

		if (assignedSellSideBookSecurities.size() > 0
				&& assignedSellSideBookSecurities.get(0).getCalcMethod() != null
				&& assignedSellSideBookSecurities.get(0).getCalcMethod().ordinal() != ((Integer) ((ComboBoxEntry) calculationComboBox.getSelectedItem())
						.getEntry()))
			dirty = true;

		if (assignedSellSideBookSecurities.size() > 0
				&& assignedSellSideBookSecurities.get(0).getCalcMethod() == null
				&& assignedSellSideBookSecurities.get(0).getSellSideBook().getCalcMethod().ordinal() != ((Integer) ((ComboBoxEntry) calculationComboBox
						.getSelectedItem()).getEntry()))
			dirty = true;

		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getBankCalendar() != null
				&& !assignedSellSideBookSecurities.get(0).getBankCalendar().equals(bankCalendarComboBox.getSelectedItem()))
			;
		dirty = true;

		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getBankCalendar() == null
				&& !assignedSellSideBookSecurities.get(0).getSellSideBook().getBankCalendar().equals(bankCalendarComboBox.getSelectedItem()))
			dirty = true;
		...
		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getValuta() != null
				&& !assignedSellSideBookSecurities.get(0).getValuta().equals(value))
			;
		dirty = true;

		if (assignedSellSideBookSecurities.size() > 0 && assignedSellSideBookSecurities.get(0).getValuta() == null
				&& !assignedSellSideBookSecurities.get(0).getSellSideBook().getValuta().equals(value))
			dirty = true;
